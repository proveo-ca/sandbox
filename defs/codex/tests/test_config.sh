...
# tests/test_config.sh - Entrypoint behaviour: smoke mode, preamble, model retirement
...
# --- Launch posture and the model-bridging retirement, through a fake codex binary ---
...
# THE RETIREMENT, ASSERTED FROM THE OUTSIDE. codex's entrypoint launches with
# `--model "$CODEX_MODEL"` and nothing else, so the fake `codex` above reports
# exactly what proveo chose. These two tests used to assert the OPPOSITE — that
# ARCHITECT_MODEL became CODEX_MODEL (bare), and that EDITOR_MODEL filled in when
# ARCHITECT_MODEL was unset. Both described the bridge table, which is deleted.
...
if echo "$RESULT" | grep -q "PASSED_MODEL=$" \
   && ! echo "$RESULT" | grep -q "PASSED_MODEL=claude-opus-5" \
   && ! echo "$RESULT" | grep -q "PASSED_MODEL=gpt-5.6"; then
  TESTS_PASSED=$((TESTS_PASSED + 1))
  printf "${GREEN}PASS${NC} [%d] no role name in .env reaches codex's --model\n" "$TESTS_RUN"
else
  TESTS_FAILED=$((TESTS_FAILED + 1))
  FAILURES+=("no role name in .env reaches codex's --model")
  printf "${RED}FAIL${NC} [%d] model bridging has grown back (output: %.300s)\n" "$TESTS_RUN" "$RESULT"
fi

# What SURVIVES: the harness's OWN variable. An operator who exports CODEX_MODEL
# is configuring codex, not asking proveo to choose — and the launch flag must
# still carry it, or the retirement took a working control with it.
cat >"$FIXTURE_DIR/.env" <<'ENVF'
ARCHITECT_MODEL=anthropic/claude-opus-5
CODEX_MODEL=explicit-model
ENVF
assert_fake "codex's own CODEX_MODEL still reaches --model" \
