...
assert_success \
  "[claudecode] ships the Kit's startup command (/usr/local/bin/proveo-seed)" \
  "$STANDALONE_IMAGE" \
  "test -x /usr/local/bin/proveo-seed"

if [ "${MCP_IMAGE_AVAILABLE:-false}" = true ]; then
  assert_success \
    "[mcp] ships the Kit's startup command (/usr/local/bin/proveo-seed)" \
    "$MCP_IMAGE" \
    "test -x /usr/local/bin/proveo-seed"
fi
