...
- A `harness.manifest` — mounts, env and posture that `proveo run <target>` applies.
...
`proveo run <target> --shell` gives troubleshooting shell access with the same mounts and env as a run.
